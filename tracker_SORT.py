...
def track(video_path, model_path=None, binary_classification=True):
    ...
    tracker = Sort()  # Ensure the SORT tracker is correctly initialized
    ...
    while cap.isOpened():
        ...
        if nrOfFrames % reducing_factor == 0:
            ...
            if nrOfFrames == 1:
                ...
                # Run SORT tracking (note: SORT expects detections in [x1, y1, x2, y2, score])
                tracked_objects = tracker.update(detections)

                # Convert detections to SORT format along with class labels
                detections, detection_labels = convert_to_sort_format(predictions)

                # For each tracked object, assign a class label by matching it with the detection of highest IoU
                for track in tracked_objects:
                    x1, y1, x2, y2, track_id = map(int, track)
                    track_box = [x1, y1, x2, y2]
                    max_iou = 0
                    assigned_label = None
                    # Try to match track with one of the detections
                    for i, det in enumerate(detections):
                        iou = compute_iou(track_box, det[:4])
                        if iou > max_iou:
                            max_iou = iou
                            assigned_label = detection_labels[i]
                    # If no good match, use previous label if exists; otherwise, set unknown (0)
                    if max_iou < 0.6:
                        assigned_label = track_labels.get(track_id, 0)
                    track_labels[track_id] = assigned_label

                    # Choose color based on assigned label
                    color = color_map.get(assigned_label, default_color)
                    ...
                    # Draw bounding box and label (with different color)
                    if binary_classification:
                        if assigned_label == 0:
                            className = "OTHER"
                        ...
                        if assigned_label == 0:
                            ...
                            className = "CAR"
                        elif assigned_label == 3:
                            className = "VAN"
                        elif assigned_label == 4:
                            className = "BUS"
                        else:
                            className = "N/A"
                    ...
                    if lastIndex > 5:
                        ...
                        if ((y_prev < yline and y_current > yline) or
                            ...
                            if binary_classification:
                                ...
                                elif assigned_label == 1:  # Large vehicle
                                    large_vehicles_set.add(track_id)
                                elif assigned_label == 2:  # Small vehicle
                                    small_vehicles_set.add(track_id)
                            else:
                                if assigned_label == 0:
                                    other_set.add(track_id)
                                elif assigned_label == 1:  # Truck
                                    trucks_set.add(track_id)
                                elif assigned_label == 2:  # Car
                                    cars_set.add(track_id)
                                elif assigned_label == 3:  # Van
                                    vans_set.add(track_id)
                                elif assigned_label == 4:  # Bus
